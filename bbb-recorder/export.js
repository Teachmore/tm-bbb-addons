...
async function main() {
    try{
        ...
        var uploadRecordingCredentialsUrl = process.argv[6]
        if(!uploadRecordingCredentialsUrl){
            console.warn("No upload credentials url provided. Convertion will be done but recording won't be uploaded")
        }
        ...
        if(convert){
            convertAndCopy(exportname, uploadRecordingCredentialsUrl)
        }else{
            copyOnly(exportname)
        }
...
function convertAndCopy(filename, uploadRecordingCredentialsUrl){
    ...
    ls.on('close', (code) => {
        ...
        if(code == 0)
        {
            ...
            addToUploadQueue(onlyfileName, copyTo, uploadRecordingCredentialsUrl)
        }
...
function addToUploadQueue(meetingId, filePath, uploadRecordingCredentialsUrl){
    const up = spawn('ruby /usr/local/recording-converter-uploader/add_to_upload_queue.rb',[meetingId, filePath, uploadRecordingCredentialsUrl], {
        shell: true
    })

    up.stdout.on('data', (data) => {
        console.log(`Add to upload queue stdout: ${data}`);
    });

    up.stderr.on('data', (data) => {
        console.error(`Add to upload queue stderr: ${data}`);
    });

    up.on('close', (code) => {
        console.log(`Add to upload queue exited with code ${code}`);
        if(code == 0)
        {
            console.log("Succesfully added to upload queue file : " + filePath)
        }
    });
}
